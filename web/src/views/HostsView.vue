...
<template>
    ...
    <div class="container" v-for="(host, mac, index) of hostData.hosts">
      <div class="row row-striped mb-3">
        ...
        <div class="col-1">
          <span v-show="!host.edit">{{host.doInstall ? "Y" : "N"}}</span>
          <input type="checkbox" v-model="host.doInstall" v-show="host.edit">
        </div>
        <div class="col-2">
    ...
    <div class="container" v-for="(host, mac, index) of hostData.unknownHosts">
      <div class="row row-striped mb-3">
        <div class="col-2">
          <span><a :href="`/ignition.json?mac=${mac}`" target="_blank">{{mac}}</a></span>
        </div>
        ...
        <div class="col-1">
          <span v-show="!host.edit">{{host.doInstall === true ? "Y" : "N"}}</span>
          <input type="checkbox" v-model="host.doInstall" v-show="host.edit">
        </div>
        <div class="col-2">
          <button class="btn btn-warning" v-show="!host.edit" v-on:click="editHost(mac, host)">edit</button>
